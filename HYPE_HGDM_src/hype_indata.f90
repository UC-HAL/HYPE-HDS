...
MODULE HYPE_INDATA
  ...
  !Copyright 2014-2015,2019-2020 SMHI
  !
...
! !>These files use sequence number.
  ...
  CHARACTER(LEN=20) :: xoms_filename(max_xoms,2) = RESHAPE(SOURCE=(/ &
...
CONTAINS
  ...
  SUBROUTINE load_xoms_files(dir,ns,bdate,edate,status)
    ...
    TYPE(DateType) fbrdate(max_xoms,2),ferdate(max_xoms,2)       !Begin and end date constrained to simulation
    ...
    DO i = 1,max_xoms
      DO j = 1,2
        CALL get_seq_filename(xoms_filename(i,j))
        INQUIRE(FILE=TRIM(dir)//TRIM(xoms_filename(i,j)),EXIST=fex(i,j))
        IF(fex(i,j))THEN
          !Check if stations are correct and present, and get their index tables.
          funit = fileunit_get()
          CALL check_station(funit,TRIM(dir)//TRIM(xoms_filename(i,j)),ns,1,subnr,   &   !Skip one row
        ...
        IF(fex(i,j))THEN
          ...
          funit = fileunit_get()
          CALL check_obs_timeperiod(funit,TRIM(dir)//TRIM(xoms_filename(i,j)),2,0,   &    !i_t=0 timesteply file
          ...
          IF(status==2)THEN  !Shorter time period
            ...
            IF(bdate.GT.fedate(i,j) .OR. edate.LT.fbdate(i,j))THEN    !no overlap
              ...
              CYCLE
            ELSE                                      !some overlap
              fbrdate(i,j) = MaxDate(bdate,fbdate(i,j))
              ferdate(i,j) = MinDate(edate,fedate(i,j))
            ENDIF
          ELSEIF(status.NE.0)THEN
            RETURN
          ENDIF
    ...
    IF(.NOT.ALLOCATED(xom))         ALLOCATE(xom(num_xoms(1),ns))
    IF(.NOT.ALLOCATED(xos))         ALLOCATE(xos(num_xoms(2),ns))
    ...
    DO i = 1,num_xomsfiles !Number of files
      DO j = 1,2
        IF(fex(i,j))THEN
          ...
          xoms_fbdate(i,j) = fbrdate(i,j)
          xoms_fedate(i,j) = ferdate(i,j)
          xoms_nstn(i,j) = numobsstn(i,j)
          IF(j==1) xomindex(i,:) = localindex(:,i,j)
          IF(j==2) xosindex(i,:) = localindex(:,i,j)
          CALL prepare_read_matrix(xoms_funit(i,j),TRIM(dir)//TRIM(xoms_filename(i,j)),2,xoms_fbdate(i,j),status)
          IF(status.NE.0) RETURN
          WRITE(6,*) 'File ready: ', TRIM(dir)//TRIM(xoms_filename(i,j))
        ENDIF
      ENDDO
    ENDDO 
  ...
  SUBROUTINE load_data_for_regression_parameter_estimate(dir,nsbase,ns,indexarray,status)
    ...
    WRITE(6,*) 'File opened: ', TRIM(dir)//'CatchGroup.txt'
    ...
    ngroup_cat=0
    DO i=1,nsbase
      READ(funit,*,ERR=101,END=102,IOSTAT=io) clust_group(i)
      IF(clust_group(i) > ngroup_cat) ngroup_cat = clust_group(i)
    ENDDO
    CLOSE(funit)
    WRITE(6,*) 'File loaded: ', TRIM(dir)//'CatchGroup.txt'
    WRITE(6,*) 'File opened: ', TRIM(dir)//'CatchDes.txt'
    OPEN(UNIT=funit,file=TRIM(dir)//'CatchDes.txt',STATUS='old',ACTION='read',ERR=103)
    READ(funit,*,ERR=106,IOSTAT=io) npc_catdes
    IF(.NOT.ALLOCATED(catdes)) ALLOCATE(catdes(nsbase,npc_catdes))
      DO i=1,nsbase
        READ(funit,*,ERR=104,END=105,IOSTAT=io)(catdes(i,j),j=1,npc_catdes)
      ENDDO
    CLOSE(funit)
    CALL fileunit_free(funit)
    WRITE(6,*) 'File loaded: ', TRIM(dir)//'CatchDes.txt'
...
105 WRITE(6,*) 'ERROR: File ending while reading: ',TRIM(dir)//'CatchDes.txt'
    status = 1
    RETURN
106 WRITE(6,*) 'ERROR: Reading number of catchment descriptors'
    WRITE(6,*) 'ERROR: io=',io
    status = 1
    RETURN
  ...
  SUBROUTINE load_regression_parameters(dir,status) 
    ...
    !>Allocate and load regression parameter coefficients
      WRITE(6,*) 'File opened: ', TRIM(dir)//'reg_par.txt'
      OPEN(unit = funit,FILE = TRIM(dir)//'reg_par.txt', STATUS = 'old', ACTION='read', ERR=100)
      READ(funit,*, ERR=101,IOSTAT=io) npar_regr
      ...
      DO l = 1,ngroup_cat 
        DO i = 1,npar_regr
          READ(funit,'(a)',ERR=101,IOSTAT=io) line
          ...
          DO j = 1,max_par
            IF(varstr==modparid(j)%shortname)THEN
              ...
              READ(funit,*,ERR=101,IOSTAT=io)line,(idx_catdes(l,i,k),k=1,nvalues)  !!read the index (position) of the catchment descriptors used for regionalization 
              EXIT
            ENDIF
          ENDDO 
        ENDDO
      ...
      CLOSE(funit)
      CALL fileunit_free(funit)
      WRITE(6,*) 'File loaded: ', TRIM(dir)//'reg_par.txt'
      RETURN
...
101 WRITE(6,*) 'ERROR: Reading file ',TRIM(dir)//'reg_par.txt'
  ...
  !------------------------------------------------------------
  SUBROUTINE set_regest_parameter(isub,ipar,rgpar,term)
  
    !Argument declarations
    INTEGER, INTENT(IN) :: isub   !<current subbasin
    INTEGER, INTENT(IN) :: ipar   !<current regression parameter
    REAL, INTENT(INOUT) :: rgpar  !<parameter value
    ...
    IF(indx_par(ipar)>0) THEN
      ...
      DO k=1,ndes_reg(clust_group(isub),indx_par(ipar))
        rgpar = rgpar + wght_catdes(clust_group(isub),indx_par(ipar),k)*catdes(isub,idx_catdes(clust_group(isub),indx_par(ipar),k))
      ENDDO
      IF(PRESENT(term)) rgpar = rgpar + term
    ENDIF
