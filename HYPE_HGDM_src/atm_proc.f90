...
MODULE ATMOSPHERIC_PROCESSES
  ...
  REAL FUNCTION get_snowfall_fraction(i,iluse,prec,temp)
    ...
    SELECT CASE(modeloption(p_snowfall))
      ...
      CASE DEFAULT  !Original model based on threshold temperatures
        ...
        ENDIF
        asnow = 1. - arain
        !asnow = 0.  !Pure rain Är inte identiskt
        !IF(prec>0.)THEN
        !  IF(temp < tt - dtt) THEN 
        !    asnow = 1. !Pure snow
        !  ELSEIF(temp > tt - dtt .AND. temp < tt + dtt) THEN
  ...
  SUBROUTINE calculate_shortwave_radiation(tmin,tmax,elev,radext,cloud,swrad,relswrad)
    ...
    REAL turbidity_angstrom             ! Ångströms cloudiness turbidity
    ...
    !Estimate turbidity from cloudiness - if available - by Ångström turbidity function
  ...
  SUBROUTINE set_precipitation_concentration(i,ns,cprec)
    ...
    IF(simulate%substance(i_in))THEN
      ...
      ELSEIF(ALLOCATED(deposition%inwetconc))THEN
        cprec(i_in) = deposition%inwetconc(i)
      ENDIF
    ENDIF
