...
TYPE assim_info_type
  ...
  INTEGER nLoc         !<number of localization matrices (1-16):   
                       !<   Depend on the number of spatial domains represented by observations and model state variables.
                       !<   In HYPE there are several spatial domains: subbasins, upstream_areas, aquifers, outregions,...,
                       !<     as well as non-spatial states such as general and land use parameters).
                       !<   In theory, nloc = ncoord_obs * ncoord_model, and ncoord_obs theoretically = ncoord_model.
                       !<   However, in practice, we re-create the coordinate system for the observations before every
                       !<     enkf analysis step, so that ncoord_obs = 1, and hence, nloc = ncoord_model = (nsub + nsub + naquifer + noutreg)
  INTEGER ncoord       !<number of spatial domains (1-4), subbasins, upstream_area, aquifers, outregions
  INTEGER coordflag(4) !<flag if a coordinate type is in use: subbasins(1), upstream_area(2), aquifers(3), outregions(4)
  ...
  !Target balance (ratio) between the R and CYY covariance matrices - used to adjust these with random numbers before inversion
  REAL covratio
     
  !G_TRANSTAT   Flag state variables should be transformed before the EnKF analysis (0=no, 1=yes)
  !G_TRANSEPS    Epsilon used for log and logit transforms for model ensembles X, F and A. Observations (Y and D) have individual transformation parameters from AssimInfo
  INTEGER transtat
  REAL traneps
  
  !Flag to stop simulation when cholesky factorisation fails (otherwise apply EnKf will skip this time step) 
...
TYPE assim_ensemble_type
  ...
  !ensemble data matrix (x), and perturbation (dx)
  REAL, ALLOCATABLE :: x(:,:)         !<ensemble data [nvar x nens]
  REAL, ALLOCATABLE :: dx(:,:)        !<perturbation data [nvar x nens]
  ...
  !integer flag to indicate transformation type (0=none,1=log,2=yeo-johnsson,3=logit)
  INTEGER           :: transform
...
TYPE assim_generation_type
  ...
  REAL              :: minsigma !<minimum sigma                      (ensgen=1,2,3,4)
  ...
  !perturbation temporal memory coefficient (fraction per timestep)
  REAL              :: tau
...
TYPE assim_data_type
  ...
  INTEGER, ALLOCATABLE :: ObsType(:)                        !vector to keep track of observation types on each row of the D, HX, and R matrices
...
!INTEGER, ALLOCATABLE :: fid_assim_ens(:,:,:) !<file unit for ensemble (statistics) output files (output,ensemble,variable/subbasin)
