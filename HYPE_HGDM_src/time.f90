...
MODULE TIMEROUTINES
  ...
  !Copyright 2011-2013,2015-2017,2019 SMHI
  !
...
CONTAINS
  ...
  !> TODO: Create a structure to hold all these.
  !>
...
!----------------------------------------------------------
  ...
  !>\brief Check if dates are within period
  !--------------------------------------------------------------------
  SUBROUTINE check_if_date_in_period(dim,n,datearray,begindate,enddate,status)  !Check for reset state
  
    !Argument declarations
    INTEGER, INTENT(IN)        :: dim      !<size of array
    INTEGER, INTENT(IN)        :: n        !<number of elements in array
    TYPE(datetype), INTENT(IN) :: datearray(dim)  !<date array to be checked
    TYPE(datetype), INTENT(IN) :: begindate  !<begin date of period
    TYPE(datetype), INTENT(IN) :: enddate  !<end date of period
    LOGICAL, INTENT(INOUT)     :: status   !<flag for any date withing period
    
    !Local variables
    INTEGER i

    DO i = 1,n
      IF(datearray(i).GE.begindate.AND.datearray(i).LE.enddate)THEN
        status = .TRUE.
        EXIT
      ENDIF
    ENDDO
    
  END SUBROUTINE check_if_date_in_period
