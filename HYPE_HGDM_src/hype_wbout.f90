...
MODULE HYPE_WATERBALANCE
  ...
  !Copyright 2014,2016-2017,2019-2020 SMHI
  !
...
!> \{
  ...
  INTEGER,PARAMETER :: w_wtrtomr   = 65  !<Index for water transfer to main river
  INTEGER,PARAMETER :: w_wtrofol   = 66  !<Index for water transfer from outlet lake
  INTEGER,PARAMETER :: w_oltombol  = 67  !<Index for flow from outlet lake to main downstream subbasin directly to olake
  INTEGER,PARAMETER :: w_oltobol   = 68  !<Index for flow from outlet lake to branch downstream directly to olake
  INTEGER,PARAMETER :: w_mrtoow    = 69  !<Index for flow from main river to outlet wetland
  INTEGER,PARAMETER :: w_owtomb    = 70  !<Index for flow from outlet wetland to main downstream subbasin
  INTEGER,PARAMETER :: w_owtob     = 71  !<Index for flow from outlet wetland to branch downstream
  INTEGER,PARAMETER :: w_iwtoir    = 72  !<Index for flow from internal wetland to local stream
  INTEGER,PARAMETER :: w_rainsrtoiw  = 73  !<Index for rain to surface runoff to iwet
  INTEGER,PARAMETER :: w_smeltsrtoiw = 74   !<Index for snow melt to surface runoff to iwet
  INTEGER,PARAMETER :: w_gmeltsrtoiw = 75  !<Index for glacier melt to surface runoff to iwet
  INTEGER,PARAMETER :: w_surfrftoiw  = 76  !<Index for surface runoff (saturated overland flow) to iwet
  INTEGER,PARAMETER :: w_tile1toiw   = 77  !<Index for tile drainage pipes runoff from soil layer 1 to iwet
  INTEGER,PARAMETER :: w_tile2toiw   = 78  !<Index for tile drainage pipes runoff from soil layer 2 to iwet
  INTEGER,PARAMETER :: w_tile3toiw   = 79  !<Index for tile drainage pipes runoff from soil layer 3 to iwet
  INTEGER,PARAMETER :: w_gwrunf1toiw = 80  !<Index for groundwater runoff from soil layer 1 to iwet
  INTEGER,PARAMETER :: w_gwrunf2toiw = 81  !<Index for groundwater runoff from soil layer 2 to iwet
  INTEGER,PARAMETER :: w_gwrunf3toiw = 82  !<Index for groundwater runoff from soil layer 3 to iwet
  INTEGER,PARAMETER :: w_preciwet    = 83  !<Index for precipitation on iwet
  INTEGER,PARAMETER :: w_precowet    = 84  !<Index for precipitation on owet
  INTEGER,PARAMETER :: w_evapiwet    = 85  !<Index for evapotranspiration iwet
  INTEGER,PARAMETER :: w_evapowet    = 86  !<Index for evapotranspiration owet
  INTEGER,PARAMETER :: num_wbflows   = 86  !<Number of water balance flows
!>\}
...
!> \{
  ...
  INTEGER,PARAMETER :: w_iwet     = 6   !<Index for local wetland
  INTEGER,PARAMETER :: w_iriver   = 7   !<Index for local stream
  INTEGER,PARAMETER :: w_ilake    = 8   !<Index for internal lake
  INTEGER,PARAMETER :: w_mriver   = 9   !<Index for main river
  INTEGER,PARAMETER :: w_olake    = 10  !<Index for outlet lake
  INTEGER,PARAMETER :: w_owet     = 11  !<Index for outlet wetland
  INTEGER,PARAMETER :: w_irrcanal = 12  !<Index for irrigation canal
  INTEGER,PARAMETER :: w_aquifer  = 13  !<Index for aquifer
  INTEGER,PARAMETER :: w_riverplain = 14  !<Index for main river floodplain
  INTEGER,PARAMETER :: w_lakeplain  = 15  !<Index for outlet lake floodplain
  INTEGER,PARAMETER :: w_wtcanal    = 16  !<Index for water transfer canal
  INTEGER,PARAMETER :: num_wbstores = 16  !<Number of water balance stores
!>\}
...
!> \{
  ...
  INTEGER,PARAMETER :: w_wdfromil  = 2   !<Index for withdrawal of water from internal lake
  INTEGER,PARAMETER :: w_wdfromdg  = 3   !<Index for withdrawal of water from deep groundwater
  INTEGER,PARAMETER :: w_wdfromol  = 4   !<Index for withdrawal of water from outlet lake as local source
  INTEGER,PARAMETER :: w_wdfrommr  = 5   !<Index for withdrawal of water from main river volume and inflow as local source
  INTEGER,PARAMETER :: w_evapirrc  = 6   !<Index for evaporation losses of (local) irrigation canal
  INTEGER,PARAMETER :: w_wdregol   = 7   !<Index for withdrawal of water from regional source (outlet lake) of this subbasin
  INTEGER,PARAMETER :: w_wdregmr   = 8   !<Index for withdrawal of water from regional source (main river volume and inflow) of this subbasin
  INTEGER,PARAMETER :: w_evapregol = 9  !<Index for evaporation losses at withdrawal of water from outlet lake (regional source)
  INTEGER,PARAMETER :: w_evapregmr = 10  !<Index for evaporation losses at withdrawal of water from river volume and inflow (regional source)
  INTEGER,PARAMETER :: w_wdoutside = 11  !<Index for withdrawal of water from outside model domain to cover all demands
  INTEGER,PARAMETER :: w_rgrwtoir  = 12  !<Index for withdrawal of water from aquifer
  INTEGER,PARAMETER :: num_wbirrflows = 12  !<Number of water balance flows regarding irrigation
!>\}
...
! !Variables for file names and file units, and for which subbasins has irrigation.
  ...
  CHARACTER(LEN=20) :: fname_wbstores(num_wbstores) = (/ &
      ...
      'WBs_iwet.txt      ','WBs_lstream.txt   ', &
      'WBs_ilake.txt     ','WBs_mriver.txt    ', &
      'WBs_olake.txt     ','WBs_owet.txt      ', &
      'WBs_irrcanal.txt  ','WBs_aquifer.txt   ', &
      'WBs_riverplain.txt','WBs_lakeplain.txt ', &
      'WBs_wtcanal.txt   '/)   !<filenames waterbalance stores
  CHARACTER(LEN=50) :: fname_wbflows(num_wbflows) = (/ &
      ...
      'WBf_evaporation_glacier_.txt                    ', &
      'WBf_watertransfer_wtcanal_mriver.txt            ', &
      'WBf_watertransfer_olake_wtcanal.txt             ', &
      'WBf_flow_olake_olake_maindownstream.txt         ', &
      'WBf_flow_olake_olake_branchdownstream.txt       ', &
      'WBf_flow_mriver_owet.txt                        ', &
      'WBf_flow_owet_mriver_maindownstream.txt         ', &
      'WBf_flow_owet_mriver_branchdownstream.txt       ', &
      'WBf_flow_iwet_lstream.txt                       ', &
      'WBf_rain_surfacerunoff__iwet.txt                ', &
      'WBf_snowmelt_surfacerunoff_snow_iwet.txt        ', &
      'WBf_melt_surfacerunoff_glacier_iwet.txt         ', &
      'WBf_satsurfaceflow_soillayer1_iwet.txt          ', &
      'WBf_tilerunoff_soillayer1_iwet.txt              ', &
      'WBf_tilerunoff_soillayer2_iwet.txt              ', &
      'WBf_tilerunoff_soillayer3_iwet.txt              ', &
      'WBf_soilrunoff_soillayer1_iwet.txt              ', &
      'WBf_soilrunoff_soillayer2_iwet.txt              ', &
      'WBf_soilrunoff_soillayer3_iwet.txt              ', &
      'WBf_precipitation__iwet.txt                     ', &
      'WBf_precipitation__owet.txt                     ', &
      'WBf_evaporation_iwet_.txt                       ', &
      'WBf_evaporation_owet_.txt                       '/) !<filenames waterbalance flows
...
CONTAINS
  ...
  SUBROUTINE prepare_waterbalance_files(dir,n,na,subid)
    ...
    INTEGER lirrindex(n)
    CHARACTER (LEN=16)   t
    CHARACTER (LEN=maxlinelength) outtxt,outtxt2,outtxt3,outtxt4
  ...
  !    CALL get_seq_filename(filename)    
      OPEN(FILE=TRIM(dir)//TRIM(filename),UNIT=funit_wbflows(i),STATUS='unknown',FORM='formatted',ACTION='write')
    ...
    DO i = 1,num_wbstores
      ...
      IF(i==w_snow.OR.i==w_glacier.OR.i==w_soil1.OR.i==w_soil2.OR.i==w_soil3.OR.  &
         i==w_iriver.OR.i==w_ilake.OR.i==w_mriver.OR.i==w_olake.OR.i==w_wtcanal.OR. &
         i==w_iwet.OR.i==w_owet.OR. &
        ...
        OPEN(FILE=TRIM(dir)//TRIM(filename),UNIT=funit_wbstores(i),STATUS='unknown',FORM='formatted',ACTION='write')
      ELSE
        CYCLE
      ENDIF
      ...
      IF(i==w_irrcanal)THEN
        WRITE(funit_wbstores(i),'(a)') outtxt2(1:lout2)
      ELSEIF(i==w_aquifer)THEN
        WRITE(funit_wbstores(i),'(a)') outtxt3(1:lout3)
      ELSEIF(i==w_riverplain.OR.i==w_lakeplain)THEN
        WRITE(funit_wbstores(i),'(a)') outtxt4(1:lout4)
      ELSE
    ...
    !    CALL get_seq_filename(filename)
        OPEN(FILE=TRIM(dir)//TRIM(filename),UNIT=funit_wbirrflows(i),STATUS='unknown',FORM='formatted',ACTION='write')
    ...
    IF(nwbfpbasin>0)THEN
      DO i = 1,num_wbfpflows
        ...
        OPEN(FILE=TRIM(dir)//TRIM(filename),UNIT=funit_wbfpflows(i),STATUS='unknown',FORM='formatted',ACTION='write')
