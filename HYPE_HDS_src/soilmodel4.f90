...
MODULE FLOODPLAIN_SOILMODEL
  ...
  USE MODVAR, ONLY: missing_value,pi,   &
       ...
       modeloption,p_floodplain,p_infiltration, &
       p_soilleakage,p_snowheat, &
       conduct,simulate,  &
       flooding,floodindex,simulatesubstances, &
       slc_mriver,slc_olake !,p_snowfalldist
  ...
  CHARACTER(LEN=30) :: errstring(2)  !error message for location of remove_water call
...
CONTAINS
  ...
  !----------------------------------------------------------------
  SUBROUTINE soilmodel_4(i,j,isoil,iluse,subid,pdayno,classarea,prec,cprec,temp, & 
       ...
       runofflows,verticalflows,cverticalflows,horizontalflows,horizontalflows2,evapsnow,totalsoilrunoff,cruralflow,snowtemp,snowsurftemp)  
    ...
    REAL snowonfp, snowmaxonfp,snowdepthonfp  !snow rescaled to non-flooded floodplain area (mm)
    REAL helparea
    ...
    REAL snowheatonfp     !snow heat content, temporary variable
    ...
    IF(modeloption(p_soilleakage)==0)THEN
      ...
      CALL add_dry_deposition_to_landclass(i,j,iluse,classdata(j)%vegtype,conduct%simN,conduct%simP, &
         classfparea*1.E-6,landpar(m_drypp,iluse),atmdepload,frozenstate,soilstate)
    ...
    IF(ffparea<classfparea)THEN
      ...
      snowheatonfp = 0.
      IF(modeloption(p_snowheat)>=1) snowheatonfp = frozenstate%snowheat(j,i)/nonflood_part
      ...
      IF(modeloption(p_floodplain)>=2)THEN
        ...
        CALL calculate_snow(i,j,basin(i)%subid,iluse,snowfall,cprec,snowonfp,   &
                ...
                snowmaxonfp,snowdepthonfp,frozenstate%snowcov(j,i),snowheatonfp,epotsnow,evapsnow,cevapsnow,effsnowcov,snowtemp,snowsurftemp,frozenstate%snowliq(j,i),snowrunoff)
      ...
      ELSEIF(modeloption(p_floodplain)==1)THEN
        ...
        snowheatonfp=0.; snowtemp=0.; snowsurftemp=0. ; snowrunoff=0.
      ...
      !Initial calcuation of liquid water fraction and frozen water volume
      ...
      IF(modeloption(p_floodplain)>=2)THEN
        CALL calculate_actual_soil_evapotranspiration(i,j,2,soilwater,temp,epotsoilwater,wpmm(:,j),  &
              fcmm(:,j),epotdist(:,j),MIN(nonflood_part,MAX(nonflood_part*(1-effsnowcov),0.)),  &
              liqfrac,soilstate,evapsoil,evapflows(1:2),cevap)
        CALL calculate_liquid_water_fraction(i,j,soilstate,frozenvol,liqfrac) !update liqfrac
      ELSEIF(modeloption(p_floodplain)==1)THEN
        evapsoil =0.; cevap = 0.
      ENDIF
      ...
      CALL calculate_soil_runoff(i,j,isoil,subid,soilwater,epmm(:,j), &
              nonflood_part*classdata(j)%streamdepth,liqfrac,soilstate,soilrunoff,csoilrunoff)
      CALL calculate_liquid_water_fraction(i,j,soilstate,frozenvol,liqfrac) !update liqfrac
      ...
      CALL calculate_tile_drainage(i,j,isoil,subid,soilwater,epmm(:,j),soildepth(:,j), &
              soilthick(:,j),nonflood_part*classdata(j)%tiledepth,rrcscorr,liqfrac,soilstate,runoffd, &
              crunoffd,cweights)
      ...
      IF(conduct%simP.OR.conduct%simS) CALL particle_processes_for_runoff(i,j,isoil,iluse,pdayno,rainfall,totalsurfaceflow,   &
              ...
              crunoff2,crunoff3,snowonfp,frostdepth,soilstate)  
      ENDIF

      !Add macropore water to soil layer with groundwater level (except the PP)
      CALL add_macropore_flow(i,j,macroflow*nonflood_part,cmacroflow, &
      ...
      IF(modeloption(p_soilleakage)==0)THEN
        ...
        IF(numsubstances>0) CALL soil_np_processes(i,j,iluse,conduct%simN,conduct%simP,conduct%simC,classarea,wpmm(:,j),  &
             pwmm(:,j),plantuptake,soilthick(:,j),genpar(m_fertdays),genpar(m_littdays),  &
        ...
        IF(conduct%simP) CALL balance_spsoil(i,j,soilthick(:,j),soilpar(m_freuc,isoil), &
      ...
      IF(simulate%substance(i_t2).AND.modeloption(p_soilleakage)==0) &
      ...
      IF(nc>0. .AND. SUM(soilrunoff)>0.) ctotalsoilrunoff = (soilrunoff(1)*crunoff1 + soilrunoff(2)*crunoff2 + soilrunoff(3)*crunoff3)/(SUM(soilrunoff))
      ...
      IF(modeloption(p_snowheat)>=1) frozenstate%snowheat(j,i) = snowheatonfp*nonflood_part
    ...
    ELSE
      ...
      ENDIF
      IF(modeloption(p_snowheat)>=1) frozenstate%snowheat(j,i)=0.
      snowtemp=0.; snowsurftemp=0.
