...
!>\brief Find the model parameters to be optimized and set the parameter
!>information variables to be used in optimization.
!>
...
SUBROUTINE find_optpar(par,parmin,parmax,parprecision,npar) 
  ...
  DO k = 1, numoptimpar
    ...
    j = parindex(k,2) 
    par(k) = parx(i,j)
    parmin(k) = optparmin(i,j)
    parmax(k) = optparmax(i,j)
    parprecision(k) = optparprecision(i,1)
  ENDDO
...
!  DO i = 1, maxoptpar
!    DO j = 1, dimpar
!!      IF(optparmin(i,j)==parx(i,j).AND.optparmax(i,j)==parx(i,j))THEN !WHy this complicated?
!      IF(optparmin(i,j)==optparmax(i,j))THEN 
!        !Not calibrated
!      ELSE
!        npar = npar + 1
!        parindex(npar,1) = i
...
!>
!>\b Consequences Module worldvar variable parindex is allocated and set
!-----------------------------------------------------------------------------------------
SUBROUTINE set_optpar_index() 

  USE WORLDVAR, ONLY : parindex, &  !OUT
                       maxoptpar, optparmin, optparmax, dimpar, numoptimpar
  
  IMPLICIT NONE

  !Argument declarations
  !REAL, INTENT(OUT)    :: parindex(numoptimpar,2)  !<Current parameter values for calibrated parameter
  !REAL, INTENT(OUT)    :: parmin(numoptimpar)  !<Lower parameter limit for calibration
  !REAL, INTENT(OUT)    :: parmax(numoptimpar)  !<Upper parameter limit for calibration
  !REAL, INTENT(OUT)    :: parprecision(numoptimpar) !<Decimal precision up to which parameter is to be calibrated
  !INTEGER, INTENT(OUT) :: npar              !<Actual number of parameters to be looped, = numoptimpar
  ...
  END SUBROUTINE set_optpar_index
...
!>Calculate criterion.
!------------------------------------------------------
SUBROUTINE run_model_crit(npar,mpar,par,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,criterion,status)
...
!-------------------------------------------------------
SUBROUTINE run_model_perf(npar,mpar,par,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate, &
...
!-------------------------------------------------------
SUBROUTINE run_model_simout(npar,mpar,par,iens,runens,allens,stateinput,frozenstate,soilstate,aquiferstate, &
  ...
  !Close output files and prepare observation for yet another simulation
  CALL close_outputfiles(nsub,naquifers,iens)
  CALL reset_observations(forcingdir,status)
  IF(status.NE.0) RETURN

  !Write additional results to files
  CALL save_mapfiles(resdir,nmapperiod,iens,runens,allens)
...
!>
!> Differential Evolution-Markov Chain (DEMC) introduces uncertainty estimate 
!> in the DE optimization algorithm by applying the Metropolis-
!> Hastings acceptance criteria, and by adding a random number in the DE proposal generation. Another
!> way of describing DEMC is that it is a simplified version of MCMC, where the DE proposal generation is 
...
!> The advantage of DEMC versus plain DE is both the possibility to get a probability based uncertainty estimate
...
!--------------------------------------------------------------------------------------------------------------------
SUBROUTINE DEMC_simulation(dir,writeall,stateinput,frozenstate,soilstate,aquiferstate,riverstate, &
...
!Run model, calculate optimization criteria
       IF(optim%task_writesim)THEN
         CALL run_model_simout(npar,numoptimpar,parprop,isim,optim%task_runens,optim%task_writesim,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,optcrit,performance,status,condcrit,condthres)
       ELSE
         CALL run_model_perf(npar,numoptimpar,parprop,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,optcrit,performance,status,condcrit,condthres)
       ENDIF
       IF(status.NE.0)THEN
...
!Print results to allsim.txt
       IF(writeall)THEN
         CALL write_simulation_results_perf_and_par(fileunit_MC,isim,numoptimpar,npar,maxperf,nacrit,optcrit,performance,parprop,writematlab,jpop,0,iacc)
       ENDIF
...
!Crossover
         CALL DEMC_crossover(jpop,npar,crossover,parprop)
!Control parameter limits
         CALL DEMC_controlparameters(npar,parprop,parmin,parmax)
!Run model, calculate optimization criteria
         IF(optim%task_writesim)THEN
           CALL run_model_simout(npar,numoptimpar,parprop,isim,optim%task_runens,optim%task_writesim,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,optcrit,performance,status,condcrit,condthres)
         ELSE
...
!Some printout?
         IF(writeall)THEN
           CALL write_simulation_results_perf_and_par(fileunit_MC,isim,numoptimpar,npar,maxperf,nacrit,optcrit,performance,parprop,writematlab,jpop,igen,iacc)
         ENDIF
!Some printout?
         CALL write_DEMC_calibration_log(igen, jpop)
       ENDDO
...
!Calculate and write MEDIAN parameterset and results to row 1 in bestMC:s, and the final populations to row 2:npop+1
     CALL DEMC_runmedianparameters(npop,npar,numoptimpar,isim+1,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,optcrit,performance,condcrit,condthres)
...
!---------------------------------------------------------------------------------------------
SUBROUTINE DEMC_runmedianparameters(npop,npar,numoptimpar,iens,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,optcrit,performance,condcrit,condthres)
...
!Run model with MEDIAN parameter set
    IF(optim%task_writesim)THEN
      CALL run_model_simout(npar,numoptimpar,medianpar,iens,optim%task_runens,optim%task_writesim,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,optcrit,performance,status,condcrit,condthres)
    ELSE
      CALL run_model_perf(npar,numoptimpar,medianpar,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,optcrit,performance,status,condcrit,condthres)
    ENDIF
    IF(status.NE.0)THEN
...
SUBROUTINE DEMC_draw_R1R2(jpop,npop,R1,R2)
  ...
  IMPLICIT NONE
    ...
    !Argument declarations
    INTEGER, INTENT(IN)  :: jpop         !<index of current population
    INTEGER, INTENT(IN)  :: npop         !<number of population
    INTEGER, INTENT(OUT) :: R1           !<index of randomly selected population 1
    INTEGER, INTENT(OUT) :: R2           !<index of randomly selected population 2
...
SUBROUTINE  DEMC_proposalgeneration(jpop,R1,R2,gamma,sigma,npar,parprop,parprec)
    ...
    !Argument declarations
    INTEGER, INTENT(IN) :: jpop         !<index of current population
    INTEGER, INTENT(IN) :: R1           !<index of randomly selected population 1
    INTEGER, INTENT(IN) :: R2           !<index of randomly selected population 2
    INTEGER, INTENT(IN) :: npar         !<Number of calibration parameters
    REAL, INTENT(IN)    :: gamma  !<mutation scaling factor
    REAL, INTENT(IN)    :: sigma  !<standard deviation of sampling error
    REAL, INTENT(INOUT) :: parprop(npar)  !<Proposal parameter vector
    REAL, INTENT(INOUT) :: parprec(npar)  !<Precision sought for parameters
...
!>With probability of crossover less than one, some parameter values
!>are kept from the previous generation.
!---------------------------------------------------------------------------------------------
SUBROUTINE DEMC_crossover(jpop,npar,crossover,parprop)
  ...
  !Argument declarations
  INTEGER, INTENT(IN) :: jpop          !<index of current population
  INTEGER, INTENT(IN) :: npar          !<Number of calibration paramters
  REAL, INTENT(IN)    :: crossover     !<Probability of mutation cross over to child
  REAL,INTENT(INOUT)  :: parprop(npar) !<Proposal parameter vector
  ...
  !Randomly decide ONE parameter that will crossover independent of probability (to get at least one crossover)
    ...
    !Loop over parameters and reject crossover if random number is above probability for crossover
    DO kpar=1,npar
...
SUBROUTINE DEMC_controlparameters(npar,parprop,parmin,parmax)
    ...
    INTEGER, INTENT(IN) :: npar         !<Number of calibration parameters
    REAL, INTENT(INOUT) :: parprop(npar) !<Proposal parameter vector
    REAL, INTENT(IN)    :: parmin(npar) !<Parameter interval lower limit
    REAL, INTENT(IN)    :: parmax(npar) !<Parameter interval upper limit
...
!Use the ratio of the deviation from the range and the range itself for reflection:
    ...
    IF(.NOT.conditional_reject)THEN
      ...
      ELSE
        ...
        IF(optim%DEMC_accprob.GT.0.)THEN
          ...
          !TODO: This can't work so well for bestcrit close to zero (that should go below). Not even checking!
...
! Start printing log file
      OPEN(UNIT = fileunit_callog, FILE = TRIM(resdir)//filename_callog, STATUS = 'replace',ACTION='write')   ! Replace calibration log file
      ...
      WRITE(fileunit_callog,'(I6,A,I6,A)') optim%DEMC_npop, ' populations (parameter sets) are runned in parallel chains which are mutated and crossed over',optim%DEMC_ngen-1,' times following the DEMC algorithm (Braak, 2006).'
      WRITE(fileunit_callog,*) ''
      WRITE(fileunit_callog,*) ''
...
!---------------------------------------------------------------------------------------------
SUBROUTINE MonteCarlo_simulation(dir,writeall,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,npar) 
  ...
  DO i = 1,optim%nruns_MC
    ...
    IF(optim%task_writesim)THEN
      CALL run_model_simout(npar,numoptimpar,par,i,optim%task_runens,optim%task_writesim,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,optcrit,performance,status)      !Evaluate the function; simulate model
    ELSE
      CALL run_model_perf(npar,numoptimpar,par,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,optcrit,performance,status)      !Evaluate the function; simulate model
    ENDIF
    IF(status.NE.0)THEN
    ...
    IF(writeall)THEN
      CALL write_simulation_results_perf_and_par(fileunit_MC,i,numoptimpar,npar,maxperf,nacrit,optcrit,performance,par,writematlab)
    ENDIF
  ENDDO
...
!---------------------------------------------------------------
SUBROUTINE bounded_MonteCarlo_simulation(taskMC,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,npar) 
  ...
  DO j = 1,optim%nruns_MCImax              
    ...
    DO i = 1,optim%nruns_MCI
      CALL get_randompar(numoptimpar,parmin,parmax,npar,par)
      IF(optim%task_writesim)THEN
        CALL run_model_simout(npar,numoptimpar,par,j*optim%nruns_MCI+i,optim%task_runens,optim%task_writesim,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,optcrit,performance,status)      !Evaluate the function; simulate model
      ELSE
        CALL run_model_perf(npar,numoptimpar,par,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,optcrit,performance,status)      !Evaluate the function; simulate model
      ENDIF
      IF(status.NE.0)THEN
...
!------------------------------------------------------------------------
SUBROUTINE param_scanning(stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
  ...
  DO counterX = 1, optim%scan_xpoints
    DO counterY = 1, optim%scan_ypoints
      ...
      CALL function_to_minim(1.0,crit,par_minvalue,direction,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
  ...
  OPEN(UNIT = fileunit_callog, FILE = TRIM(resdir)//filename_callog, STATUS = 'replace',ACTION='write') !Start calibration log file
...
!-------------------------------------------------------------------------
SUBROUTINE stage_MonteCarlo(stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
  ...
  DO stageCounter = 0,optim%nruns_stages-1                  !Loop through the stages (counter offset by 1 for convenience)
    ...
    IF(stageCounter == 0) THEN
      ...
      CALL stage_MonteCarlo_core_function(writeall,1,stageCounter+1,amountStageRuns,  &
        par_center,parRad,par_minvalue,par_maxvalue,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
    ELSE
      ...
      DO bestCounter = 1,optim%nruns_best                       !Update center for this stage
        ...
        CALL stage_MonteCarlo_core_function(writeall,bestCounter,stageCounter+1, &
           amountStageRuns,par_center,parRadStage,par_minvalue,par_maxvalue,stateinput,frozenstate, &
...
SUBROUTINE stage_MonteCarlo_core_function(writeall,icenter,istage, &
                           nruns,parCenter,parRadStage,parMin,parMax,stateinput, &
                           frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
    ...
    USE WORLDVAR, ONLY : nacrit, maxperf,           &
                         ...
                         allocate_MCvariables,      &
                         optim, numoptimpar
    USE DATAMODULE, ONLY : write_simulation_results_perf_and_par
    ...
    DO irun = 1,nruns
      ...
      IF(optim%cal_debugCase == 0) THEN
        CALL run_model_perf(numoptimpar,numoptimpar,par,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,optcrit,performance,status)      ! For the randomized par-vector at hand, run the model and evaluate the criteria function
        IF(status.NE.0) THEN
          WRITE(6,*) 'Error stage MC simulation in run',irun,' of stage',istage
          STOP 1
        ENDIF
      ELSE
        CALL function_to_minim(1.,optcrit,parCenter,par-parCenter,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
      ENDIF
      ...
      IF(writeall) THEN
        ...
        CALL write_simulation_results_perf_and_par(fileunit_MC,iterations_done,numoptimpar,numoptimpar,maxperf,nacrit,optcrit,performance,par,writematlab)
      ENDIF
...
!-----------------------------------------------
SUBROUTINE linesearch_methods_calibration(npar,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,par)
  ...
  IF(optim%task_stpstDesc .OR. optim%task_DFP .OR. optim%task_BFGS) THEN  !Start the quasi-Newton gradient-based DFP or BFGS optimisation procedure, or steepest descent (implemented as particular case of quasi-Newton)  [Fred, 15.07.10]
    ...
    ENDIF
    CALL quasiNewton_algorithm(critLastVect,parLastTable,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
  ENDIF
  ...
  IF(optim%task_BrentNew) THEN                  !Start optimisation by Brent routine, with new line search method  [Fred, 28.11.10]
    ...
    CALL new_Brent_method(critLastVect,parLastTable,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
  ENDIF
...
!-------------------------------------------------------------------------------------------
SUBROUTINE linesearch_HYSS(xMinIn,xMaxIn,refPar,parPrecis,direction,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,xBest,fBest,xGood,fGood)
  ...
  REAL, INTENT(IN), OPTIONAL :: xGood    !<best previous point
  REAL, INTENT(IN), OPTIONAL :: fGood    !<function value at best previous point
  ...
  CALL function_to_minim(xCurrent,fCurrent,refPar,direction,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)   ! Get corresponding function value f(xCurrent)
  ...
  IF(PRESENT(xGood))THEN
    IF(fCurrent<fGood)THEN
      xBefore = xCurrent          ! 2 previous current points and corresp. function values are retained for parabolic approx. Initialize as xCurrent and xGood
      fBefore = fCurrent
      xBefore2 = xGood
      fBefore2 = fGood
    ELSE
  ...
  DO WHILE((intervalTol - (b - a)/2) < ABS(xCurrent - xMean) .AND. iterCounter < maxIter .AND. (.NOT.stopFlag))
     ...
     CALL function_to_minim(xNew,fNew,refPar,direction,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)         ! ...and the corresponding function value
...
!>Check if wanted parameter precision is reached. The line search interval is 
!>less than precision for all parameters to be optimized in this line search.
!------------------------------------------------------------------------------
SUBROUTINE linesearch_check_decim(intLength, parPrecis, direction, stopFlag)
  ...
  REAL, INTENT(IN)     :: parPrecis(numoptimpar)  !<precision asked for
  REAL, INTENT(IN)     :: direction(numoptimpar)  !<current direction of search
  LOGICAL, INTENT(OUT) :: stopFlag    !<flag for line serach interval less than precision asked
  
  !Local variables
  INTEGER dimCounter, sumpar
  REAL    stuff
  ...
  DO dimCounter = 1, numoptimpar
    IF(direction(dimCounter)/=0.)THEN
      ...
      IF(intLength < stuff) sumpar = sumpar + 1
    ELSE
      sumpar = sumpar + 1
    ENDIF
  ENDDO

  IF(sumpar == numoptimpar) THEN
    stopFlag = .TRUE.
...
!-------------------------------------------------------------------
SUBROUTINE function_to_minim(lambda,flambda,refPar,direction,stateinput,   &
                             frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
  ...
  CASE(0)       !Not a debug: run HYPE, return criteria
    CALL run_model_crit(numoptimpar,numoptimpar,newPar,stateinput, &
                        frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate, &
...
!----------------------------------------------------------
SUBROUTINE new_Brent_method(critLastVect,parLastTable,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
  ...
  lambdaBest = 0
  direction = 0.
  CALL function_to_minim(lambdaBest,critBest,par,direction,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
  ...
  DO
    ...
    DO dimCounter = 1,numoptimpar
      ...
      CALL linesearch_HYSS(0.0,1.0,parStep,parPrecis,direction,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,lambdaBest,critBest)
    ...
    IF(optim%Brent_diagonalStep)THEN
      ...
      IF(SUM(direction)==0)THEN    !check if optimum reached (no changes) !CP120918
        ...
        CALL linesearch_methods_interruptor_printandstop_core_function(stopFlag, 6)                               !Print in hyss.log
        IF(optim%cal_log) CALL linesearch_methods_interruptor_printandstop_core_function(stopFlag, fileunit_callog)    !Print in calibration.log if requested
        !DO dimCounter = 1,numoptimpar   !If no improvement at all in calibration, still need to set original par-value as best for final simulation and respar.txt printout
          parLastTable(1,:) = par
        !ENDDO
        EXIT   
      ELSE
        ...
        CALL linesearch_HYSS(-lambdaNeg,lambdaPos,iterStartPar,parPrecis,direction,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,lambdaBest,critBest)
...
SUBROUTINE initialize_Brent_calibrationlog()
  ...
  WRITE(fileunit_callog, '(A, A)') 'Model area: ', trim(modeldir)
  WRITE(fileunit_callog, *) ''
  WRITE(fileunit_callog, '(A)') 'Calibration settings:'
...
SUBROUTINE write_Brent_calibrationlog(par, parStep, direction, newPar, lambdaBest, critBest)
  ...
  DO counter = 1, numoptimpar
     WRITE(fileunit_callog, '(A,F17.9, A, F17.9, A, F17.9, A, F17.9)', advance='no') '   ',par(counter), '          ', parStep(counter), '    ', direction(counter), '     ', newPar(counter)
     WRITE(fileunit_callog, '(A)') ' '
  ENDDO
  WRITE(fileunit_callog, *) ''
...
!----------------------------------------------------------------------
SUBROUTINE quasiNewton_algorithm(critLastVect,parLastTable,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
  ...
  ENDDO
  CALL function_to_minim(lambdaBest,critBest,par,direction,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
  CALL grad_criterium_function(par,parMin,parMax,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,gradVect,gradNorm)
  ...
  DO WHILE(gradNorm > optim%QN_flatTol)
     ...
     CALL direction_multiplier(par,parMin,parMax,parPrecis,direction,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,lambdaBest,critBest,lastCritBest)

     !>\li Compute the new numerical gradient of the gradient function
     parNew = par + lambdaBest*direction
     CALL grad_criterium_function(parNew,parMin,parMax,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,gradVectNew,gradNorm)
...
!! Note: parameter order according to optpar.txt required (not checked) and 
!! only the parameters with (larger than zero) intervals. It checks if
...
SUBROUTINE load_qNstarting_vector(parMin, parMax, par)
  ...
  DO lineCounter = 1, numoptimpar
    ...
    pos = 1
    CALL read_next_codestr_on_line(18000,10,pos,charLine,str,nostrfound,'err')
    READ(charLine(pos:50),*) par(lineCounter)
...
!-------------------------------------------------------------------------------
SUBROUTINE grad_criterium_function(par,parMin,parMax,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,gradVect,gradNorm)
  ...
  REAL, INTENT(IN)  :: parMin(numoptimpar)  !<Parameter interval lower limit
  REAL, INTENT(IN)  :: parMax(numoptimpar)  !<Parameter interval upper limit
  LOGICAL, INTENT(IN) :: stateinput       !<code for reading state
  TYPE(snowicestatetype),INTENT(INOUT) :: frozenstate   !<Snow and ice states
  TYPE(soilstatetype),INTENT(INOUT)  :: soilstate   !<Soil states
  TYPE(aquiferstatetype),INTENT(INOUT) :: aquiferstate  !<Aquifer states
  ...
  DO dimCounter = 1, numoptimpar
     ...
     DO stencilCounter = 1,9
       ...
       ELSE
         ...
         CALL function_to_minim(lambdaVal,optcrit,par,directionVect,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
...
!------------------------------------------------------------------------------------------
SUBROUTINE direction_multiplier(par,parMin,parMax,parPrecis,direction,stateinput,  &
                frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,lambdaBest,critBest,prevCritBest)
  ...
  CALL linesearch_HYSS(0.0,maxLambda,par,parPrecis,direction,stateinput,frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate,lambdaBest,critBest,0.0,prevCritBest)
