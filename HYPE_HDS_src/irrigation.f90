...
MODULE IRRIGATION_MODULE
  ...
  !Copyright 2011-2015,2017-2020 SMHI
  !
  ...
  CHARACTER(LEN=44) :: errstring(2)  !error message for location of remove_water call
  PARAMETER (errstring = (/'irrigation evaporation loss, local source   ', & !1
                           'irrigation evaporation loss, regional source'/))
...
CONTAINS
  ...
  SUBROUTINE initiate_irrigation(n,nc)
    ...
    IF(.NOT.ALLOCATED(fieldneed))THEN
      ...
      !>\li Set regional irrigation coupling and sources
      CALL set_regional_irrigation(n)

    ENDIF

  END SUBROUTINE initiate_irrigation

  !>\brief Determine which classes are irrigated. An irrigated class 
  !>is determined by CropData columns plantday and imm_start.
  !>
      ...
      DO ir=1,SIZE(cropindex,2)
        ...
        IF(classdata(j)%crop2>0)THEN
          IF(cropindex(classdata(j)%crop2,ir)>0)THEN
            ...
            IF(cropirrdata(cropindex(classdata(j)%crop2,ir))%imm_start>0) irrtype(j)=1
          ENDIF
        ENDIF
        IF(irrtype(j)==1)EXIT
      ENDDO
  ...
  END SUBROUTINE find_irrigated_classes
  ...
  SUBROUTINE set_regional_irrigation(n)
    ...
    !Local variables
    INTEGER nmax      !maximum number of subbasins from a regional source
    INTEGER i,j
    ...
    !>Set index of regional source for subbasins with regional source, 
    !>and count the number of collectors from each source.
    DO j = 1, dimirr
    ...
    !>Determine collectors for all subbasins, i.e. which
  ...
  !---------------------------------------------------------------------------
  SUBROUTINE calculate_irrigation_water_demand(i,j,area,sswcorrpar,immdeppar,  &
                        iwdfracpar,wdpar,soil,wpl,fcl,epl,epot,iwdv)
    ...
    REAL pottrans  !potential transpiration (mm)
    ...
    IF(cropiindex==0)THEN   !Temporary tests. These should be checked in the beginning, not here every time.
      WRITE(6,*) 'ERROR: Crop missing in CropData; cropid',classdata(j)%crop,' in class',j
      WRITE(6,*) 'ERROR: for region',basin(i)%region,' used for subbasin',basin(i)%subid
      STOP 1
    ENDIF

    !Irrigation season?
    IF(.NOT.in_season_end(cropirrdata(cropiindex)%plantingdayno,cropirrdata(cropiindex)%season_end)) RETURN

    !Immersion crop and season?
    immcrop = .FALSE.
    immseason = .FALSE.
    IF(cropirrdata(cropiindex)%imm_start>0) immcrop = .TRUE.
    IF(immcrop) immseason = in_season_end(cropirrdata(cropiindex)%imm_start,cropirrdata(cropiindex)%imm_end)
    ...
    ELSE  
      ...
      CALL calculate_kcb(cropiindex,dayno,kcb)  
  ...
  !>\brief Add irrigation water to the top soil layer of a class.
  !!
  !>\b Reference ModelDescription Water management (Irrigation - Irrigation water application)
  !--------------------------------------------------------------------
  SUBROUTINE apply_irrigation(i,j,soilstate,appirr,appirrmass)
  ...
  !      !apply2 = 0 ! Elenius testing irrigate from above
  ...
  SUBROUTINE calculate_irrigation(i,na,irrigationpar,q,lakew,riverv,aquiferv, &
    ...
    USE MODVAR, ONLY : nsub,         &
                       ...
                       numsubstances,&
                       irrunlimited
    USE SOIL_PROCESSES, ONLY : set_evaporation_concentrations
    ...
    REAL tempvol           !temporary variable holding abstraction volume for calculating new concentration after evaporation
    REAL cevap(numsubstances)     !temporary variable holding concentration of evaporation for calculating new concentration of abstracted water after evaporation
...
!          clocal_add_irr = soilstate%conc(:,2,j,i)*epotdist(2,j) + soilstate%conc(:,1,j,i)*epotdist(1,j)     !Concentration of the added water = concentration of the soil layers where the water is put (i.e. no conc. change), scaled by the proportion of water ending up in each layer
          clocal_add_irr = soilstate%conc(:,1,j,i)     !Concentration of the added water = concentration of the soil layers where the water is put (i.e. no conc. change)
      ...
      !!Case of source limited irrigation
...
!             aquiferv(ia) = aquiferv(ia) - gwabstr
       ...
       IF(local_add_irr >0.)THEN
         ...
         CALL set_evaporation_concentrations(clocal_add_irr,cevap)    ! Calculate evapo-concentration
         tempvol = local_add_irr/local_efficiency
         CALL remove_water(tempvol,numsubstances,clocal_add_irr,tempvol*(1.-local_efficiency),cevap,ierr)   !Calculate new concentration due to evapo-concentration (i.e. NP more concentrated due to water evaporating)
         IF(ierr.NE.0) CALL error_remove_water(errstring(1),basin(i)%subid,i,0)
...
!       ENDIF
    ...
    IF(totaldemand_regsrc(i)>0 .AND. regionaldemand(i)==0.)THEN             !Jafet: Checks for regionaldemand(i) because otherwise you may take water from what was protected by pirrgpar and pirrspar above
       ...
       IF(regional_add_irr>0.)THEN
         ...
         ENDIF
          ...
          !cregional_add_irr = (cregional_dw_irr*regional_dw_irr + cq*regional_sw_irr + cregional_rv_irr*regional_rv_irr)/regional_add_irr  !made safe for c=NaN och Inf
          cregional_add_irr = 0.
          IF(regional_dw_irr>0.) cregional_add_irr = cregional_add_irr + cregional_dw_irr*regional_dw_irr
          IF(regional_sw_irr>0.) cregional_add_irr = cregional_add_irr + cq*regional_sw_irr
          IF(regional_rv_irr>0.) cregional_add_irr = cregional_add_irr + cregional_rv_irr*regional_rv_irr
          cregional_add_irr = cregional_add_irr/regional_add_irr
  ...
  !             cwatertofield = cregional_add_irr / (irrigationsystem(irrindex(iadd))%reg_eff * irrigationsystem(irrindex(iadd))%local_eff)   ! Concentration of watertofield (evaporation-> concentration both regionally and locally).
               cwatertofield = cregional_add_irr
               CALL set_evaporation_concentrations(cwatertofield,cevap)    ! Calculate evapo-concentration
               tempvol = regionaldemand(iadd) * regional_add_irr_factor
               CALL remove_water(tempvol,numsubstances,cwatertofield,tempvol*(1.-(irrigationsystem(irrindex(iadd))%reg_eff * irrigationsystem(irrindex(iadd))%local_eff)),cevap,ierr)   !Calculate new concentration due to evapo-concentration (i.e. NP more concentrated due to water evaporating)
               IF(ierr.NE.0) CALL error_remove_water(errstring(2),basin(i)%subid,i,0)
               ...
               ENDDO
             ELSE
               wbrirrflows(1,iadd) = 0.
               wbrirrflows(2,iadd) = 0.               
             ENDIF
          ENDDO
  ...
  SUBROUTINE irrigation_abstraction_sink(conc,cirrsinkpar,abstr,sinkmass)
    ...
    IF(simulate%substance(i_pp)) conc(i_pp) = conc(i_pp)*(1-cirrsinkpar)
    IF(simulate%substance(i_on)) conc(i_on) = conc(i_on)*(1-cirrsinkpar)
