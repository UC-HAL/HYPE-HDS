...
CONTAINS
  ...
  SUBROUTINE initiate_soil_npc_state(initN,initP,initC,maxlayers,soilstate)
    ...
    IF(initN)THEN
      ...
      DO j = 1,nclass   !Initiate with top layer concentration
        soilstate%humusN(:,j,1) = landpar(m_humusN0,classdata(j)%luse) * soilthick(:,j)
        soilstate%fastN(:,j,1)  = genpar(m_fastN0) * soilthick(:,j)
        IF(maxlayers>1)THEN   !Adjust according to soillayer decline in concentration        
          IF(landpar(m_hNhalf,classdata(j)%luse)>0.)THEN
            halfHN = log(2.) / landpar(m_hNhalf,classdata(j)%luse)
            soilstate%humusN(2,j,1) = soilstate%humusN(2,j,1) * EXP(-halfHN * ((soilthick(1,j)/2.) + (soilthick(2,j)/2.)))
            soilstate%humusN(3,j,1) = soilstate%humusN(3,j,1) * EXP(-halfHN * ((soilthick(1,j)/2.) + soilthick(2,j) +  (soilthick(3,j)/2.)))
          ENDIF
        ENDIF
    ...
    IF(initP)THEN
      DO i = 1,nsub
        ...
        ENDIF
        DO j = 1,nclass   
          soilstate%humusP(:,j,i) = phoscorr*landpar(m_humusP0,classdata(j)%luse) * soilthick(:,j) !Initiate with top layer concentration
          IF(landpar(m_hPhalf,classdata(j)%luse)>0.)THEN
            halfHP = log(2.) / landpar(m_hPhalf,classdata(j)%luse)    
            soilstate%humusP(2,j,i) = soilstate%humusP(2,j,i) * EXP(-halfHP * ((soilthick(1,j)/2.) + (soilthick(2,j)/2.)))           
            soilstate%humusP(3,j,i) = soilstate%humusP(3,j,i) * EXP(-halfHP * ((soilthick(1,j)/2.) + soilthick(2,j) +  (soilthick(3,j)/2.)))  
          ENDIF
          soilstate%fastP(:,j,i)  = phoscorr*genpar(m_fastP0) * soilthick(:,j)
          IF(landpar(m_partP0,classdata(j)%luse)>0)THEN
            soilstate%partP(:,j,i)  = phoscorr*landpar(m_partP0,classdata(j)%luse) * soilthick(:,j)
            IF(landpar(m_pPhalf,classdata(j)%luse)>0.)THEN
              halfPP = log(2.) / landpar(m_pPhalf,classdata(j)%luse)
              soilstate%partP(2,j,i) = soilstate%partP(2,j,i) * EXP(-halfPP * ((soilthick(1,j)/2.) +  (soilthick(2,j)/2.)))            
              soilstate%partP(3,j,i) = soilstate%partP(3,j,i) * EXP(-halfPP * ((soilthick(1,j)/2.) + soilthick(2,j) + (soilthick(3,j)/2.)))            
            ENDIF
          ELSE
            soilstate%partP(1,j,i)  = phoscorr*landpar(m_partP1,classdata(j)%luse) * soilthick(1,j)
            soilstate%partP(2,j,i)  = phoscorr*landpar(m_partP2,classdata(j)%luse) * soilthick(2,j)
            soilstate%partP(3,j,i)  = phoscorr*landpar(m_partP3,classdata(j)%luse) * soilthick(3,j)
          ENDIF
        ENDDO
      ENDDO
    ENDIF
    ...
    IF(initC)THEN
      DO j = 1,nclass
        help(1) = landpar(m_humusC1,classdata(j)%luse) * soilthick(1,j)
        help(2) = landpar(m_humusC2,classdata(j)%luse) * soilthick(2,j)
        help(3) = landpar(m_humusC3,classdata(j)%luse) * soilthick(3,j)
      ...
      ENDDO
      DO j = 1,nclass
        help(1) = landpar(m_fastC1,classdata(j)%luse) * soilthick(1,j)
        help(2) = landpar(m_fastC2,classdata(j)%luse) * soilthick(2,j)
        help(3) = landpar(m_fastC3,classdata(j)%luse) * soilthick(3,j)
  ...
  SUBROUTINE set_class_precipitation_concentration_and_load(ns,area,precorg,temp,prec,cprecorg,cprec,precload)
    ...
    IF(prec>0)THEN
      ...
      ELSE
        cprec = cprecorg
      ENDIF    
      !IF(simulate%substance(i_in)) precload(i_in) = cprec(i_in)*prec*area
      !IF(simulate%substance(i_sp)) precload(i_sp) = cprec(i_sp)*prec*area

      !Temp.conc. in prec = temp(air)|max=0 for landclasses [DG/JS Temp.model May-2013]
      IF(simulate%substance(i_t2)) cprec(i_t2)= MAX(0.,temp) 
      
      IF(numsubstances>0) precload(:) = cprec(:)*prec*area
    ENDIF
  ...
  !---------------------------------------------------------------------
  SUBROUTINE add_dry_deposition_to_landclass(i,j,iluse,veg,conductN,conductP, &
                                  areaij,drypp,source,frozenstate,soilstate)
    ...
    sourceDD = 0.
    IF(ALLOCATED(deposition%indryload).AND.simulate%substance(i_in) .AND. veg>0) sourceDD(i_in) = deposition%indryload(i,veg)
  ...
  !------------------------------------------------------------
  SUBROUTINE calculate_plant(i,j,temp,daylength,common_uptake,miscstate)

    USE MODVAR, ONLY : cropdata,        &
                       find_croppart,   &
                       ...
                       prevdoy, &
                       in_season_end
    USE TIMEROUTINES, ONLY : numdays_of_year
    ...
    !Local variables
    INTEGER k,kcrop
    REAL bd2,bd3,bd5                 !cultivation dates
    REAL uptakeN
    REAL part                           !part of class area that crop covers
    REAL sl1part,PNratio                !uptake parameters
    REAL tmpfcn

    !>\b Algorithm \n
    !>Check if plant uptake to be calculated, and set default value.
    common_uptake = 0.
    IF(.NOT.conduct%simN .AND. .NOT.conduct%simP) RETURN
    ...
    DO kcrop = 1, 2                             !main and secondary crop
      ...
      IF(part>0)THEN
        ...
        IF(modeloption(p_growthstart)==1)THEN
          IF(dayno == cropdata(k)%firstday .OR. (cropdata(k)%firstday==0.AND.dayno==1)) THEN   !reset day before start of accumulation or beginning of year (default)
          ...
          ELSE
            bd2 = cropdata(k)%baredayno3       !no uptake yet
          ENDIF
        ENDIF
        
        !>Calculate nitrogen uptake for growing season
        uptakeN = 0.
        IF(in_season_end(NINT(bd2),NINT(bd3)))THEN
          IF(dayno>=bd2)THEN
            uptakeN = plant_growth(cropdata(k)%uptake1,cropdata(k)%uptake2,cropdata(k)%uptake3,REAL(dayno-bd2))
          ELSE
            uptakeN = plant_growth(cropdata(k)%uptake1,cropdata(k)%uptake2,cropdata(k)%uptake3,REAL(dayno+prevdoy-bd2))
          ENDIF
        ENDIF

        !>Calculate nitrogen uptake for autumn sowed crop before winter set in
        IF(bd5>0)THEN
          IF(NINT(bd5)>=181.AND.in_season_end(NINT(bd5+1),numdays_of_year(currentdate)))THEN  !to end of year
            IF(temp<5.) THEN       
              tmpfcn=0.
            ELSE
              tmpfcn = MIN(1., (1./20.)*temp-(5./20.)) 
            ENDIF
            uptakeN = tmpfcn * plant_growth(cropdata(k)%uptake1,cropdata(k)%uptake2,cropdata(k)%uptake3,REAL(dayno-(bd5+25)))
          ELSEIF(NINT(bd5)<181.AND.in_season_end(NINT(bd5+1),numdays_of_year(currentdate)-184))THEN !to 30/6
            IF(temp<5.) THEN       
              tmpfcn=0.
            ELSE
              tmpfcn = MIN(1., (1./20.)*temp-(5./20.)) 
        ...
        !>Calculate phosphorus uptake, and sum uptake of crop to common uptake function for class
  ...
  SUBROUTINE crop_sources(i,j,calcN,calcP,calcC,ndays,ldays,areaij,thickness,soilstate,sources)
    ...
    DO kcrop = 1,2                             !main and secondary crop
      ...
      IF(part>0)THEN
        ...
        IF(in_season_period(cropdata(k)%fertday1,fertperiod))THEN
          ...
          ENDIF
        ENDIF
        IF(in_season_period(cropdata(k)%fertday2,fertperiod))THEN
          IF(thickness(2)>0)THEN
          ...
          ENDIF
        ENDIF
        IF(in_season_period(cropdata(k)%manday1,fertperiod))THEN
          IF(thickness(2)>0)THEN
            IF(calcN)THEN
          ...
          ENDIF
        ENDIF
        IF(in_season_period(cropdata(k)%manday2,fertperiod))THEN
          IF(thickness(2)>0)THEN
            IF(calcN)THEN
        ...
        IF(cropdata(k)%resdayno==0 .OR.   &
           in_season_period(cropdata(k)%resdayno,litterperiod))THEN
          IF(thickness(2)>0)THEN
            IF(calcN)THEN
  ...
  SUBROUTINE particle_processes_for_runoff(i,j,isoil,iluse,dayno,prec,surfacerunoff,macroporflow,    &
                          ...
                          crunoff3,snow,frostdepth,soilstate)
    ...
    USE HYPEVARIABLES, ONLY : m_soilcoh,m_soilerod,m_sreroexp,           &
                              ...
                              m_ppenrmax,m_ppenrstab,m_ppenrflow
    ...
    CASE(1)
      CALL calculate_hbvsed_erosion(i,prec,landpar(m_erodluse,iluse),soilpar(m_erodsoil,isoil), &   !calculate eroded sediment basedon HBV-sed
                      genpar(m_erodslope),genpar(m_erodexp),genpar(m_erodindex),snow,frostdepth,erodedsed)
    END SELECT
    !>Calculate particulate phosphorus of eroded sediment
    IF(simulate%substance(i_pp)) CALL calculate_pp_transport(erodingflow,erodedsed,soilpar(m_ppenrmax,isoil),genpar(m_ppenrstab),genpar(m_ppenrflow),soilstate%partP(1,j,i),soilstate%humusP(1,j,i),soilthick(1,j),fracminP,erodedP)
    ...
    IF(erodedsed>0. .AND. surfacerunoff>0.) THEN     !erodedP goes back to soil if no surface runoff
      ...
      IF(simulate%substance(i_pp)) surfrppconc = srfilt * erodedP / erodingflow
      IF(simulate%substance(i_ss)) surfrssconc = srfilt * erodedsed / erodingflow
      IF(tilerunoff>0.) THEN       !only when runoffd>0  will PP from macropor reach runoff
        IF(macroporflow>0. )THEN
          IF(simulate%substance(i_pp)) macroppconc =  soilpar(m_macfilt,isoil) * erodedP / erodingflow
          IF(simulate%substance(i_ss)) macrossconc =  soilpar(m_macfilt,isoil) * erodedsed / erodingflow
        ENDIF
      ENDIF
      IF(i_pp>0)THEN
    ...
    !Temporary pool delays surface runoff eroded SS and PP and tile runoff SS and PP. 
    !The phosphorus/sediment is released during high total runoff equally distributed over all runoff paths.
!    IF(genpar(m_pprelmax)>0. .AND. totflow>0.)THEN
    IF(totflow>0.)THEN
      IF(simulate%substance(i_pp))THEN
        ...
        ELSE
          PPrel = soilstate%PPrelpool(j,i)
          soilstate%PPrelpool(j,i) = 0.
        ENDIF
      ...
      IF(simulate%substance(i_ss))THEN
        ...
        ELSE
          SSrel = soilstate%Srelpool(j,i) 
          soilstate%Srelpool(j,i) = 0.
        ENDIF
  ...
  !>Calculate the plant cover of a class that will limit erosion of the soil. 
  !>Crop cover shelter from rain and ground cover from surface runoff.
  !!
  !> \b Reference ModelDescription Chapter Nitrogen and phosphorus in land routines (Vegetation 
  !! and soil surface processes - Erosion calculations, Crop cover and ground cover)
  !------------------------------------------------------------------
  SUBROUTINE get_current_crop_and_ground_cover(i,j, &
                               total_cropcover,total_groundcover)
    ...
    INTEGER, INTENT(IN) :: i             !<index of current subbasin
    INTEGER, INTENT(IN) :: j             !<index of current class
    REAL, INTENT(OUT)   :: total_cropcover     !<total crop cover of class (-)
    REAL, INTENT(OUT)   :: total_groundcover   !<total ground cover of class (-)

    !Local variables
    LOGICAL yearround
    INTEGER k       !crop/row in CropData
    INTEGER kcrop   !main (1) or secondary (2) crop
    REAL cropcover, groundcover   !cover by crop
    REAL part       !fraction of class area covered by the crop
    REAL gperiod,gperiod2 !growth period (days)
    ...
    DO kcrop = 1, 2
      ...
      IF(part>0)THEN
        ...
        yearround = .FALSE.
        IF(bd1==0 .AND. bd4==0) yearround = .TRUE.
        IF(bd1>0.AND.bd4==0) bd4 = bd1    !TODO: to remove bd1 from input, but so far set them here
        IF(yearround) THEN                   !year-round-crop
        ...
        ELSE                                           !spring, winter and row crops
          ...
          IF(bd2>bd3)  gperiod = REAL(bd3 + prevdoy - bd2)/2.
          maxday1 = INT(bd2 + gperiod)   !day of maximum crop and ground cover in summer, save these instead when reading?
          gperiod = REAL(maxday1 - bd2)
          IF(maxday1>prevdoy) maxday1 = maxday1 - prevdoy
          IF(bd5>=181)THEN
            maxday2 = INT(bd5 + REAL(prevdoy-bd5)/2.)   !day of maximum crop and ground cover for winter crops in autumn (northern hemispere)
          ELSE
          ...
          IF(bd5 > 0)THEN                              !winter crop
            IF(maxday2/=bd2.AND.in_season_end(maxday2,bd2-1))THEN
              cropcover   = cropdata(k)%ccmax2
              groundcover = cropdata(k)%gcmax2
            ELSEIF(in_season_period(bd2,NINT(gperiod)))THEN
              IF(dayno>=bd2)THEN
                cropcover   = cropdata(k)%ccmax2 + (cropdata(k)%ccmax1-cropdata(k)%ccmax2)*((dayno - REAL(bd2)) / gperiod)
                groundcover = cropdata(k)%gcmax2 + (cropdata(k)%gcmax1-cropdata(k)%gcmax2)*((dayno - REAL(bd2)) / gperiod)
              ELSE    !bd2 and dayno on different sides of new year
                cropcover   = cropdata(k)%ccmax2 + (cropdata(k)%ccmax1-cropdata(k)%ccmax2)*((dayno + prevdoy - REAL(bd2)) / gperiod) 
                groundcover = cropdata(k)%gcmax2 + (cropdata(k)%gcmax1-cropdata(k)%gcmax2)*((dayno + prevdoy - REAL(bd2)) / gperiod)
              ENDIF
            ELSEIF(maxday1/=bd3.AND.in_season_end(maxday1,bd3-1))THEN
              cropcover   = cropdata(k)%ccmax1
              groundcover = cropdata(k)%gcmax1
            ELSEIF(bd3/=bd4.AND.in_season_end(bd3,bd4-1))THEN
              cropcover   = cropdata(k)%gcmax1
              groundcover = cropdata(k)%gcmax1    
            ELSEIF(bd4/=bd5.AND.in_season_end(bd4,bd5-1))THEN
              cropcover = 0.
              groundcover = 0.
            ELSEIF(in_season_period(bd5,NINT(gperiod2)))THEN
              IF(dayno>=bd5)THEN
                cropcover   = cropdata(k)%ccmax2 * ((dayno - REAL(bd5)) / gperiod2)
                groundcover = cropdata(k)%gcmax2 * ((dayno - REAL(bd5)) / gperiod2)
              ELSE
                cropcover   = cropdata(k)%ccmax2 * ((dayno + prevdoy - REAL(bd5)) / gperiod2)
                groundcover = cropdata(k)%gcmax2 * ((dayno + prevdoy - REAL(bd5)) / gperiod2)
              ENDIF
            ENDIF
          ELSE  !bd4>0                                       !spring crop (or row crop)
            IF(bd4/=bd2.AND.in_season_end(bd4,bd2-1))THEN
              cropcover   = 0.
              groundcover = 0.
            ELSEIF(in_season_period(bd2,NINT(gperiod)))THEN
              IF(dayno>=bd2)THEN
                cropcover   = cropdata(k)%ccmax1 * ((dayno - REAL(bd2)) / gperiod) 
                groundcover = cropdata(k)%gcmax1 * ((dayno - REAL(bd2)) / gperiod)
              ELSE
                cropcover   = cropdata(k)%ccmax1 * ((dayno + prevdoy - REAL(bd2)) / gperiod)
                groundcover = cropdata(k)%gcmax1 * ((dayno + prevdoy - REAL(bd2)) / gperiod)
              ENDIF
            ELSEIF(maxday1/=bd3.AND.in_season_end(maxday1,bd3-1))THEN
              cropcover   = cropdata(k)%ccmax1
              groundcover = cropdata(k)%gcmax1
          ...
          ENDIF
        ENDIF !bd5>0
        total_cropcover   = total_cropcover * (1. - part * cropcover)
        total_groundcover = total_groundcover * (1. - part * groundcover)
      ENDIF !part>0
    ENDDO
    total_cropcover   = 1. - total_cropcover
    total_groundcover = 1. - total_groundcover 
    
  END SUBROUTINE get_current_crop_and_ground_cover
  ...
  !! and soil surface processes - Erosion calculations)
  !------------------------------------------------------------------
  SUBROUTINE calculate_pp_transport(flow,erodedsed,parmax,parstab,parflowstab,soilPartP,soilHumusP,thickness,fracminP,erodedP)
    
    USE HYPEVARIABLES, ONLY : bulkdensity
    
    !Argument declarations
    REAL, INTENT(IN)  :: flow        !<fast flow (mm/timestep)
    REAL, INTENT(IN)  :: erodedsed   !<mobilised sediment (kg/km2)
    REAL, INTENT(IN)  :: parmax      !<parameter for maximum enrichment (-)    
    REAL, INTENT(IN)  :: parstab     !<parameter for minimum enrichment (stable level) (-)
    REAL, INTENT(IN)  :: parflowstab !<parameter for flow when stable level is reached (mm/timestep)    
    ...
    !>\b Algorithm
    !>If no phosphorus in soil, return with no erosion
    fracminP = 0.
    erodedP = 0.
    IF(soilPartP + soilHumusP<=0.) RETURN
    
    !>Calculate enrichment factor - finer soil particles are more likely to be eroded and contain more P per weight unit
    IF(flow>0.)THEN     
      IF(flow>parflowstab)THEN 
        enrichment = parstab
      ELSE 
        enrichment = parmax - (parmax-parstab)* flow / parflowstab
      ENDIF
    ELSE
      enrichment = 0.
    ENDIF    
  ...
  !------------------------------------------------------------------------------
  SUBROUTINE soil_np_processes(i,j,iluse,calcN,calcP,calcC,area,wp,pw, &
                      common_uptake,thickness,ndays,ldays,nitrification,  &
    ...
    REAL, INTENT(IN)    :: pw(maxsoillayers)        !<pore wolume (mm)
    ...
    CALL crop_sources(i,j,calcN,calcP,calcC,ndays,ldays,area,thickness,soilstate,cropsources)                                              !fertilizer and residues, also orgC
    CALL soil_pool_transformations(i,j,calcN,calcP,wp,pw,thickness,source,pardisfN,  &
    ...
    IF(simulate%substance(i_in)) nitrification = source(i_in)
    CALL plant_uptake(calcN,calcP,common_uptake,wp,thickness,soilstate%water(:,j,i),soilstate%conc(:,:,j,i),sink)    
    IF(simulate%substance(i_in)) cropuptake = sink(i_in)
    CALL calculate_soil_profile_denitrification(calcN,i,j,iluse,pw,thickness,denpar1,soilstate,denitrification)
  ...
  !> \b Reference ModelDescription Chapter Nitrogen and phosphorus in land routines (Soil processes)
  !------------------------------------------------------------------------------
  SUBROUTINE soil_pool_transformations(i,j,calcN,calcP,wp,pw,thickness,source,        &
    ...
    REAL, INTENT(IN)    :: wp(maxsoillayers)        !<water content at wilting point (mm)
    REAL, INTENT(IN)    :: pw(maxsoillayers)        !<pore volumeater (mm)
    REAL, INTENT(IN)    :: thickness(maxsoillayers) !<thickness of soil layers
    ...
    smfcn(1) = moisturefactor(sm(1),wp(1),pw(1),thickness(1),satact,thetapow,thetalow,thetaupp)
    smfcn(2) = moisturefactor(sm(2),wp(2),pw(2),thickness(2),satact,thetapow,thetalow,thetaupp)
    smfcn(3) = moisturefactor(sm(3),wp(3),pw(3),thickness(3),satact,thetapow,thetalow,thetaupp)

    !Calculation of transformations
    degradhN = 0.    !Degradation of humusN to fastN
    transfN = 0.     !Transformation of fastN to inorganic N
    dissolfN = 0.    !Transformation of fastN to dissolved organic N
    dissolhN = 0.    !Transformation of humusN to dissolved organic N
    IF(calcN)THEN
      degradhN(:) = degrhNpar * tmpfcn(:) * smfcn(:) * soilstate%humusN(:,j,i)
      dissolhN(:) = pardishN * tmpfcn(:) * smfcn(:) * soilstate%humusN(:,j,i)
      transfN(:) = minfNpar * tmpfcn(:) * smfcn(:) * soilstate%fastN(:,j,i)
      ...
      IF(thickness(3)>0)THEN
        ...
        CALL production_pool(maxsoillayers,DONpool(:),dissolhN(:)+dissolfN(:))
      ...
      ELSEIF(thickness(2)>0)THEN
        ...
        CALL production_pool(2,DONpool(1:2),dissolhN(1:2)+dissolfN(1:2))
      ...
      ELSE
        ...
        CALL production_pool(1,DONpool(1),dissolhN(1:1)+dissolfN(1:1))
      ...
      IF(thickness(2)>0)THEN
        CALL new_concentration(inorgNpool(2),soilstate%water(2,j,i),soilstate%conc(i_in,2,j,i))
        CALL new_concentration(DONpool(2),soilstate%water(2,j,i),soilstate%conc(i_on,2,j,i))
      ENDIF
      IF(thickness(3)>0)THEN
        CALL new_concentration(inorgNpool(3),soilstate%water(3,j,i),soilstate%conc(i_in,3,j,i))
        CALL new_concentration(DONpool(3),soilstate%water(3,j,i),soilstate%conc(i_on,3,j,i))
      ENDIF
    ENDIF
    
    degradhP = 0.    !Degradation of humusP to fastP           
    transfP = 0.    !Transformation of fastP to SRP
    dissolfP = 0.    !Transformation of fastP to dissolved organic P (DOP/PP)
    dissolhP = 0.    !Transformation of humusP to dissolved organic P
    IF(calcP)THEN
    ...
    IF(calcP)THEN
      ...
      IF(thickness(2)>0)THEN
        CALL new_concentration(SRPpool(2),soilstate%water(2,j,i),soilstate%conc(i_sp,2,j,i))
        CALL new_concentration(DOPpool(2),soilstate%water(2,j,i),soilstate%conc(i_pp,2,j,i))
      ENDIF
      IF(thickness(3)>0)THEN 
        CALL new_concentration(SRPpool(3),soilstate%water(3,j,i),soilstate%conc(i_sp,3,j,i))
        CALL new_concentration(DOPpool(3),soilstate%water(3,j,i),soilstate%conc(i_pp,3,j,i))
      ENDIF
    ENDIF
  ...
  SUBROUTINE plant_uptake(calcN,calcP,common_uptake,wp,thickness,soil,csoil,sink)
    ...
    ENDIF
    maxpooluptake = 0.
    IF(soil(1)>0.) maxpooluptake(1) = (soil(1)-wp(1))/soil(1)                      !Maximum uptake dependent on soil moisture
    IF(thickness(2)>0. .AND. soil(2)>0.) maxpooluptake(2) = (soil(2)-wp(2))/soil(2)
  ...
  SUBROUTINE calculate_soil_profile_denitrification(calcN,i,j,iluse,porevolume,thickness,denpar_basic,soilstate,denitrification)
    ...
    INTEGER, INTENT(IN) :: j                        !<index of class
    INTEGER, INTENT(IN) :: iluse                    !<index of land use
    REAL, INTENT(IN)    :: porevolume(maxsoillayers)        !<pore volume (mm)
    REAL, INTENT(IN)    :: thickness(maxsoillayers) !<thickness of soil layers
  ...
  SUBROUTINE  freundlich(iin,jin,lin,PoolPP,SRP_Conc,Vol,LayerThick,Kfr,Nfr,Kadsdes)
    ...
    IF(conc_sol<=0.)THEN
      nfrloc = 1.
      IF(conductwarning) WRITE(6,*) 'Warning: soil partP <=0. Freundlich will give error, take shortcut.'
    ENDIF
    ...
    IF(ABS(PPequi_conc - conc_sol) > 10E-6)THEN
      ...
      IF(-help>poolPP.OR.SRP_conc<(help/ vol))THEN
        ...
        IF(conductwarning) WRITE(6,*) 'Warning: freundlich flow adjusted, was larger than pool'
      ENDIF
  ...
  !-----------------------------------------------------------------
  SUBROUTINE local_diffuse_source(i,j,pw,classarea,soilstate,ruralaload,upwardflow,ruralflow,cruralflow,alldissolved)
    ...
    LOGICAL,OPTIONAL, INTENT(IN) :: alldissolved     !<flag for all dissolved load
    ...
    LOGICAL ppdissolved !flag for adding PP dissolved in soil water
    LOGICAL addruralflow !flag for rural flow to this soil
    ...
    !Find if rural flow is to be added
    addruralflow = .FALSE.
    IF(conductbasinlocsoil)THEN
      IF(load(i)%locsoil>0.) addruralflow = .TRUE.
    ELSE
      IF(genpar(m_locsoil)>0) addruralflow = .TRUE.
    ENDIF
    
    !>If rural load...
    IF(addruralflow)THEN
!    IF(genpar(m_locsoil)>0)THEN
      IF(PRESENT(alldissolved)) ppdissolved = alldissolved
       IF(conductbasinlocsoil)THEN
        qin = load(i)%locsoil * load(i)%volloc / landarea(i) * 1000.     !->mm/d !fel vid korta tidsteg!
      ELSE
...
!     qin = genpar(m_locsoil) * load(i)%volloc / landarea(i) * 1000.     !->mm/d !fel vid korta tidsteg!
      IF(qin>0)THEN
        ...
        !>Set concentration of rural inflow to soil (IN,ON, and SP)
        cin = load(i)%locconc
        IF(simulate%substance(i_pp))THEN
          IF(.NOT.ppdissolved)THEN
            cin(i_pp) = 0. !PP to fastP instead (below)
          ELSE
            ruralaload(i_pp) = cin(i_pp) * qin
          ENDIF
        ENDIF
        IF(i_in>0)THEN 
  ...
  !-----------------------------------------------------------------
  SUBROUTINE soil_carbon_processes(i,j,wp,fc,pw,thickness,klh,klo,&
  ...
  SUBROUTINE doc_percolation_reduction(n,conc,kpar,temp,soilm,wiltp,porew,thickness)
    ...
    !No carbon or percolation reduction modelled
    IF (i_oc == 0) RETURN
    IF (kpar == 0) RETURN
  ...
  SUBROUTINE onpp_percolation_reduction(n,conc,kparN,kparP)         
    ...
    !No nitrogen and phosphorus modelled or no percolation reduction modelled.
    IF(i_pp == 0.AND.i_on == 0) RETURN
    IF(kparN == 0. .AND. kparP == 0.) RETURN
  ...
  !-----------------------------------------------------------------------
  SUBROUTINE class_riparian_zone_processes(i,j,n,q,conc1,conc2,conc3,oldgrw, &
       kpar,stemp,apar,spar,gwat,t10,t20,soilm,wiltp,porew,depthm)

    USE MODVAR, ONLY : i_oc,    &
         maxsoillayers,basin,classbasin
    ...
    help = kpar * ((basin(i)%elev+classbasin(i,j)%deltah)/1000.) * grwfcn * seafcn * smfcn
