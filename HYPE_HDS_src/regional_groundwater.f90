...
MODULE REGIONAL_GROUNDWATER_MODULE
  ...
  USE GENERAL_FUNCTIONS, ONLY : exponential_decay
  ...
  CHARACTER(LEN=49) :: errstring(5)  !error message for location of remove_water call
...
CONTAINS
  ...
  !--------------------------------------------------------------------------
  SUBROUTINE initiate_regional_groundwater_flow(n,ns,cb_ilake,cb_olake,cb_lriver,cb_mriver,cb_iwet,cb_owet)
    ...
    INTEGER, INTENT(IN)   :: cb_iwet     !<slc-index for internal wetland
    INTEGER, INTENT(IN)   :: cb_owet     !<slc-index for outlet wetland
    ...
    IF(cb_olake>0 .OR. cb_ilake>0 .OR. cb_lriver>0 .OR. cb_mriver>0)THEN 
       ...
       IF(cb_iwet>0) lpart = lpart + classbasin(1:n,cb_iwet)%part
       IF(cb_owet>0) lpart = lpart + classbasin(1:n,cb_owet)%part
  ...
  SUBROUTINE initiate_aquifer_state(aquiferstate)
    ...
    DO ia=1,naquifers
      ...
      IF(simulate%substance(i_in)) aquiferstate%conc(i_in,ia) = aquifer(ia)%conc_IN   !mg/L
      IF(simulate%substance(i_sp)) aquiferstate%conc(i_sp,ia) = aquifer(ia)%conc_SP   !mg/L
      IF(simulate%substance(i_t2)) aquiferstate%conc(i_t2,ia) = aquifer(ia)%temperature   !degree C
    ENDDO
      ...
      IF(naquifers==0.OR.path(i)%rechargebasin)THEN
        DO j = 1,nclass
          IF(j==slc_ilake .OR. j==slc_olake .OR.  &   !only land classes
             j==slc_lriver .OR. j==slc_mriver .OR. &
             j==slc_iwet .OR. j==slc_owet) CYCLE
  ...
  SUBROUTINE calculate_soillayer_groundwaterflow_removal(i,j,subid,n,soil,  &
    ...
    IF(soil>plantwater)THEN
      ...
      IF(n>0) CALL set_percolation_concentration(n,csoil,cflow) !Reduce the concentration of the flows
  ...
  !>\brief Calculate regional groundwater flow from main river. 
  ...
  SUBROUTINE calculate_river_groundwaterflow_removal(i,j,subid,n,riverstate,flow)
   ...
   USE MODVAR, ONLY : basin, &
    ...
    !Local variables
    INTEGER, PARAMETER :: itype=2   !main river
    INTEGER status
    
    INTEGER isoil, l
    REAL cflow(n)
    REAL rcgrwj
    REAL rivervol,outflow
    REAL accsubst(n)
    REAL fractionw,fractionlast,notused   !fractions of volume in river compartments
    ...
    IF(path(i)%rechargebasin)THEN
      ...
      ALLOCATE(fractionqueue(ttstep(itype,i)))
      CALL calculate_water_fractions(ttstep(itype,i),ttpart(itype,i),riverstate%water(itype,i),0.,riverstate%qqueue(1:ttstep(itype,i)+1,itype,i),rivervol,fractionw,notused,fractionqueue,fractionlast)
      IF(rivervol<=0.)THEN
      ...
      IF(flow>0.)THEN
        IF(n>0) accsubst = cgrwflow(:,i)*grwflow(i)
        !>Remove flow from river damping box...
        IF(fractionw>0)THEN
          outflow = flow*fractionw
          IF(n>0) CALL set_percolation_concentration(n,riverstate%conc(:,itype,i),cflow)  !remove_water can't handle leaving substances behind
          IF(outflow<riverstate%water(itype,i))THEN
            CALL remove_water(riverstate%water(itype,i),n,riverstate%conc(:,itype,i),outflow,cflow,status)
            IF(status.NE.0) CALL error_remove_water(errstring(3),subid,i,itype)
          ELSE
            riverstate%water(itype,i)=0.
            IF(n>0) riverstate%conc(:,itype,i)=0.
          ENDIF
          IF(n>0) accsubst = accsubst + cflow * outflow*1.E3
          grwflow(i) = grwflow(i) + outflow*1.E3   !mm*m2
        ENDIF
        !>...and from the queue proportional to their volume.
        DO l = 1,ttstep(itype,i)
          IF(fractionqueue(l)>0)THEN
            outflow = flow*fractionqueue(l)
            IF(n>0) CALL set_percolation_concentration(n,riverstate%cqueue(:,l,itype,i),cflow)  !remove_water can't handle leaving substances behind
            IF(outflow<riverstate%qqueue(l,itype,i))THEN
              CALL remove_water(riverstate%qqueue(l,itype,i),n,riverstate%cqueue(:,l,itype,i),outflow,cflow,status)
              IF(status.NE.0) CALL error_remove_water(errstring(4),subid,i,itype)
            ELSE
              riverstate%qqueue(l,itype,i)=0.
              IF(n>0) riverstate%cqueue(:,l,itype,i)=0.
            ENDIF
            IF(n>0) accsubst = accsubst + cflow * outflow*1.E3
            grwflow(i) = grwflow(i) + outflow*1.E3   !mm*m2
          ENDIF
        ...
        IF(fractionlast>0)THEN
          l = ttstep(itype,i) + 1
          outflow = flow*fractionlast   !this is what to remove from last qqueue to get correct remaining river outflow, not the water leaving to deeper groundwater
          IF(n>0) CALL set_percolation_concentration(n,riverstate%cqueue(:,l,itype,i),cflow)  !remove_water can't handle leaving substances behind
          IF(outflow<riverstate%qqueue(l,itype,i))THEN
            CALL remove_water(riverstate%qqueue(l,itype,i),n,riverstate%cqueue(:,l,itype,i),outflow,cflow,status)
            IF(status.NE.0) CALL error_remove_water(errstring(4),subid,i,itype)
          ELSE
            ...
            IF(n>0) riverstate%cqueue(:,l,itype,i)=0.
          ENDIF
          outflow = outflow * ttpart(itype,i)   !Water that actually flow down.
          IF(n>0) accsubst = accsubst + cflow * outflow*1.E3
          grwflow(i) = grwflow(i) + outflow*1.E3   !mm*m2
        ENDIF

        !>Calculate concentration of flow
        IF(n>0) cgrwflow(:,i) = accsubst/grwflow(i)
      ENDIF
    ENDIF
    IF(ALLOCATED(fractionqueue)) DEALLOCATE(fractionqueue)
   ...
   USE MODVAR, ONLY : basin, &
    ...
    IF(path(i)%rechargebasin)THEN
      ...
      IF(flow>0.)THEN
        ...
        IF(n>0) CALL set_percolation_concentration(n,miscstate%cfloodwater(:,itype,i),cflow)
        ...
        ELSE
          ...
          IF(n>0) miscstate%cfloodwater(:,itype,i)=0.
        ENDIF
        IF(n>0) accsubst = accsubst + cflow * flow*1.E3
  ...
  SUBROUTINE set_percolation_concentration(n,waterconc,percconc)
    ...
    USE MODVAR, ONLY : i_on,i_pp,i_oc, &
                       modeloption,p_soilleakage,simulate
    ...
    IF(n==0)RETURN
    percconc = waterconc
    IF(.NOT.(modeloption(p_soilleakage)==2 .OR. modeloption(p_soilleakage)==3))THEN    !With soil load model, all substances follow to aquifer
      IF(simulate%substance(i_on)) percconc(i_on) = 0.
      IF(simulate%substance(i_pp)) percconc(i_pp) = 0.
      IF(simulate%substance(i_oc)) percconc(i_oc) = 0.
    ENDIF
  ...
  SUBROUTINE calculate_aquifer(aquiferstate,aqoutflow,remflow2,aquiferirrloss)
    ...
    USE HYPEVARIABLES, ONLY : m_denitaq,m_hsatINsoil, &
          m_ssdecaq,m_aedecaq,m_spdecaq,m_ppdecaq,m_t1decaq,m_ondecaq,m_ocdecaq
    USE MODVAR, ONLY : genpar,i_ae,i_ss,i_sp,i_pp,i_t1,i_on,i_oc,timesteps_per_day
    ...
    ts = 1. / REAL(timesteps_per_day)  !time in days
    DO ia=1,naquifers
      ...
      IF(i_ss>0.AND.genpar(m_ssdecaq)>0.)THEN
        aquiferstate%conc(i_ss,ia) = aquiferstate%conc(i_ss,ia) * exponential_decay(ts,genpar(m_ssdecaq))
        !aquiferstate%conc(i_ss,ia) = aquiferstate%conc(i_ss,ia) * exponential_decay(1.0,genpar(m_ssdecaq))
      ENDIF
      IF(i_ae>0.AND.genpar(m_aedecaq)>0.)THEN
        aquiferstate%conc(i_ae,ia) = aquiferstate%conc(i_ae,ia) * exponential_decay(ts,genpar(m_aedecaq))
      ENDIF
      IF(i_sp>0.AND.genpar(m_spdecaq)>0.)THEN  
        aquiferstate%conc(i_sp,ia) = aquiferstate%conc(i_sp,ia) * exponential_decay(ts,genpar(m_spdecaq))
      ENDIF 
      IF(i_pp>0.AND.genpar(m_ppdecaq)>0.)THEN  
        aquiferstate%conc(i_pp,ia) = aquiferstate%conc(i_pp,ia) * exponential_decay(ts,genpar(m_ppdecaq))
      ENDIF
      IF(i_on>0.AND.genpar(m_ondecaq)>0.)THEN  
        aquiferstate%conc(i_on,ia) = aquiferstate%conc(i_on,ia) * exponential_decay(ts,genpar(m_ondecaq))
      ENDIF
      IF(i_oc>0.AND.genpar(m_ocdecaq)>0.)THEN  
        aquiferstate%conc(i_oc,ia) = aquiferstate%conc(i_oc,ia) * exponential_decay(ts,genpar(m_ocdecaq))
      ENDIF
      IF(i_t1>0.AND.genpar(m_t1decaq)>0.)THEN  
        aquiferstate%conc(i_t1,ia) = aquiferstate%conc(i_t1,ia) * exponential_decay(ts,genpar(m_t1decaq))
      ENDIF 
    ENDDO   
  ...
  SUBROUTINE calculate_aquifer_outflow(ia,aquiferstate,aqoutflow,caqoutflow)
    ...
    aqoutflow = outpar * (aquiferstate%water(ia) - aquifer(ia)%passivevol)  
...
!      aqwl(ia) = aquifer(ia)%passivedep + aquiferstate%water(ia)/aquifer(ia)%porosity/aquifer(ia)%area
      aqwl(ia) = aquifer(ia)%reference + aquiferstate%water(ia)/aquifer(ia)%porosity/aquifer(ia)%area
    ENDDO
  ...
  !----------------------------------------------
  FUNCTION calculate_aquifer_conc(ns,na,subst,aquiferstate) RESULT(subconc)

    !Argument declarations  
    INTEGER, INTENT(IN) :: ns             !<Number of subbasins
    INTEGER, INTENT(IN) :: na             !<Number of aquifers
    INTEGER, INTENT(IN) :: subst          !<Substance
    TYPE(aquiferstatetype),INTENT(IN) :: aquiferstate  !<Aquifer state
    REAL subconc(ns)                      !<Aquifer concentration (mg/L or other)
    
    INTEGER i,ia    !loop-index, i-subbasin,ia-aquifer 
    REAL aqconc(na) !aquifer water depth (m)
    
    DO ia=1,na
      aqconc(ia) = aquiferstate%conc(subst,ia)
    ENDDO
    subconc = missing_value
    DO i=1,ns
      IF(path(i)%rechargebasin .OR. path(i)%recievefraction>0) subconc(i) = aqconc(path(i)%aquid)
    ENDDO

  END FUNCTION calculate_aquifer_conc
